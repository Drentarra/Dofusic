...
The job adds only `actions: read` to the necessary `contents: write`, `id-token: write`, `attestations: write`; top-level `permissions: {}` and checkout `persist-credentials: false` remain. This necessary deviation from Task 5's three permissions lets SignPath read job details and download the artifact. [SignPath GitHub integration](https://docs.signpath.io/trusted-build-systems/github) requires Actions/Contents read for `github-token`; the [Download an artifact REST API](https://docs.github.com/en/rest/actions/artifacts#download-an-artifact) explicitly requires Actions read, including artifact downloads from this public repository.

The pinned action receives the ephemeral `github.token` of the release job. GitHub token permissions cannot be reduced per step. Supplying a separate read-only token would need another PAT/App secret or restructuring build/sign/publication into separate jobs. No extra PAT/App identity or architecture is introduced here. The vendor action consequently receives the publication job's write permissions too; a later job split could reduce this exposure.
